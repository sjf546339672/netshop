...
from django.db.transaction import atomic  # 事务  原子性
...
def test_model():
    with open('utils/jiukuaijiu.json') as fr:
        ...
        for data in datas:
            ...
            for goods in _goods:
                good = Goods.objects.create(gname=goods['goodsname'],
                ...
                for _color in goods['colors']:
                    color = Color.objects.create(colorname=_color[0],
                                                 colorurl=_color[1])
                    colors.append(color)
                ...
                for _spec in goods['specs']:
                    ...
                    for img in _spec[1]:
                        GoodsDetail.objects.create(goods=good,
                                                   gdname=goodsdetails,
                                                   gdurl=img)

                for c in colors:
                    for s in sizes:
                        Inventory.objects.create(count=100, goods=good,
                                                 color=c, size=s)
