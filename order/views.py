...
alipay = AliPay(appid="2021000117601490",
                app_notify_url="127.0.0.1:9005/order/checkPay/",
                app_private_key_path="order/keys/my_private_key.txt",
                alipay_public_key_path="order/keys/my_public_key.txt",
                return_url="127.0.0.1:9005/order/checkPay/",
                debug=True)
...
class ToPayView(View):
    ...
    def get(self, request):
        ...
        print(cartitems)
        orderItemList = [OrderItem.objects.create(order=orderObj, **item) for
                         item in cartitems if item]
        print("----=======", request.GET.get('totalPrice'))
