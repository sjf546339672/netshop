...
class SessionCartManager(CartManager):
    ...
    def add(self, goodsid, colorid, sizeid, count, *args, **kwargs):
        ...
        if key in self.session[self.cart_name]:
            ...
            self.session[self.cart_name][key] = CartItem(goodsid=goodsid,
                                                         colorid=colorid,
                                                         sizeid=sizeid,
                                                         count=count)
    ...
    def migrateSession2DB(self):
        if 'user' in self.session:
            ...
            for cartitem in self.queryAll():
                if CartItem.objects.filter(goodsid=cartitem.goodsid,
                                           colorid=cartitem.colorid,
                                           sizeid=cartitem.sizeid).count() == 0:
                    cartitem.user = user
                    cartitem.save()
                else:
                    item = CartItem.objects.get(goodsid=cartitem.goodsid,
                                                colorid=cartitem.colorid,
...
class DBCartManger(CartManager):
    ...
    def add(self, goodsid, colorid, sizeid, count, *args, **kwargs):
        if self.user.cartitem_set.filter(goodsid=goodsid,
                                         colorid=colorid,
                                         sizeid=sizeid).count() == 1:
            self.update(goodsid, colorid, sizeid, count, *args, **kwargs)
        else:
            CartItem.objects.create(goodsid=goodsid,  colorid=colorid,
                                    sizeid=sizeid,  count=count,
                                    user=self.user)

    def delete(self, goodsid, colorid, sizeid, *args, **kwargs):
        self.user.cartitem_set.filter(goodsid=goodsid, colorid=colorid,
                                      sizeid=sizeid).update(count=0,
                                                            isdelete=True)

    def update(self, goodsid, colorid, sizeid, step, *args, **kwargs):
        self.user.cartitem_set.filter(goodsid=goodsid,
                                      colorid=colorid,
                                      sizeid=sizeid).update(
